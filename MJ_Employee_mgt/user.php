...
if (isset($_POST['saveBTN'])) {
	...
	if (
		!empty($u_id) || !empty($u_name) || !empty($u_ph) || !empty($u_email) || !empty($u_address)
	) {
		$insert = "insert into users values
				('$u_id', '$u_name', '$u_ph', '$u_email', '$u_address')";
...
if (isset($_POST['searchBTN'])) {
	...
	if (mysqli_num_rows($result) > 0) {
		while ($row = mysqli_fetch_assoc($result)) {
			...
			$u__name = $row['u_name'];
			$u__ph = $row['u_ph'];
			$u__email = $row['u_email'];
			$u__address = $row['u_address'];
...
if (isset($_POST['updateBTN'])) {
	...
	if (
		...
		$update = "update users set u_name='$u_name', u_ph='$u_ph', u_email = '$u_email', u_address = '$u_address' where u_id='$u_id'";
	...
	<nav class="navbar navbar-expand-lg navbar-dark" style="background-color: #AB274F;">
		...
		<div class="collapse navbar-collapse" id="navbarSupportedContent">
			...
			<div class="form-inline my-2 my-lg-0">
				<form method="POST">
					<input class="form-control mr-sm-2" type="search" name="u_search" placeholder="Search" aria-label="Search" />
			...
			<div class="row no-gutters mx-5 mb-4 px-3 py-3">
				<div class="form-row">
					<div class="form-group col-md-6 form__group field">
						<input type="text" class="form-control form__field" id="usrID" name="u_id" value="<?= $u__id ?>" placeholder="User ID" required />
						<label class="form__label" for="usrID">User ID</label>
					</div>
					<div class="form-group col-md-6 form__group field">
						<input type="text" class="form-control form__field" id="uname" name="u_name" value="<?= $u__name ?>" placeholder="User Name" required />
						<label class="form__label" for="uname">User Name</label>
					</div>
					<div class="form-group col-md-6 form__group field">
						<input type="text" class="form-control form__field" id="ph_no" name="u_ph" value="<?= $u__ph ?>" placeholder="Phone Number" required />
						<label class="form__label" for="ph_no">Phone Number</label>
					</div>
					<div class="form-group col-md-6 form__group field">
						<input type="email" class="form-control form__field" id="email" name="u_email" value="<?= $u__email ?>" placeholder="Email ID" required />
						<label class="form__label" for="email">E-mail ID</label>
					</div>
					<div class="form-group col-md-12 form__group field">
						<textarea class="form-control form__field" id="inputAddress" rows="2" name="u_address" value="<?= $u__address ?>" placeholder="1234 Main Street" required><?= $u__address ?></textarea>
